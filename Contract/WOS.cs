...
namespace WOS
{
	public class WOS : SmartContract
    {
		public static class Const
        {
			public const float shipMoveSpeed = 150;
			public const float raidShipCargoRate = 0.1f;
			public const float safeZoneLine = 1567;
			public const float damagePerAttackShip = 0.1f;
			public const float energyCostPerLyExpand = 0.01f;
			public const float nukemissSpeed = 3600;
			public const float nukeRadius = 120;
			public const float totalStarCnt = 1000;
			public const float starCargoC0 = 100;
			public const float starArmyC0 = 10;
			public const float starPeriodTimestamp = 0;
			public static User[] allUserList = new User[];
			public static WormHole[] allWormHoles = new WormHole[];
		...
		public static object Main(string operation, params object[] args)
        {
			if (Runtime.Trigger == TriggerType.Verification){
				...
				if (operation == "attackStar")
                {
                    return attackStar(args);
                }

				if (operation == "attackWormHole")
                {
                    return attackWormHole(args);
                }

				if (operation == "collectWormHoleMoney")
                {
                    return collectWormHoleMoney(args);
                }
        ...
        public static BigInteger GetNumWormHoles()
        {
			return GetGlobal().numWormHoles;
        }
